...
class WordBean {
	...
	public void replaceAllText(String toFindText, String newText) {
		Dispatch.call(selection, "HomeKey",new Variant(6)); 	//到文档开头部
		while (find(toFindText)) {
			if(toFindText.indexOf("proptype")!=-1){
				Dispatch.put(selection, "Text", toFindText.substring(toFindText.length()-2,toFindText.length()-1));
				Dispatch.put(Dispatch.get(selection, "Font").toDispatch(), "Underline", newText);
			}else {
				Dispatch.put(selection, "Text", newText);
			}
			Dispatch.call(selection, "MoveRight");
	 ...
	 *            图片路径
	 */
	public void replaceAllImageNew(String toFindText, String imagePath) {
		Dispatch.call(selection, "HomeKey",new Variant(6)); 	//到文档开头部
		while (find(toFindText)) {
			Dispatch.call(Dispatch.get(selection, "InLineShapes").toDispatch(),
					"AddPicture", imagePath);
...
//			Dispatch.put(picture, "Height", new Variant(420)); // 图片的高度
...
//			Dispatch.call(picture, "MoveRight");
		}
	}
	/** */
	/**
...
public class JacobToWord {
	...
	public static void createANewFileTest() {
		...
		//wordBean.insertImage("D:" + File.separator + "icons.png"); // 插入图片
		// 如果 ，想保存文件，下面三句
	...
	public static void openAnExistsFileTest() {
		WordBean wordBean = new WordBean(true);
		wordBean.openDocument("d:\\b.doc");
		wordBean.insertImage("D:" + File.separator + "icons.png"); // 插入图片(注意刚打开的word
		// ，光标处于开头，故，图片在最前方插入)
		wordBean.save("d:\\b.doc");
	...
	public static void insertFormatStr(String str) {
		...
		wordBean.createNewDocument();// 创建一个新文档
		wordBean.insertText(str);// 插入一个段落，对其中的字体进行了设置
	}

	public static void replaceStr(String file, String s, String d, String newfile) {
		WordBean wordBean = new WordBean();
		wordBean.openDocument(file);
		wordBean.replaceAllText(s, d);
		wordBean.save(file);
		wordBean.closeDocument();
		wordBean.close();
	}

	public static void download(String file, Map<String, Object> map){
		...
		try {
			...
			for (Entry<String, Object> entry : map.entrySet()) {
				...
				if(entry.getValue().toString().startsWith("IMG:")) {
					if(!Util.getInstance().isEmpty(entry.getValue().toString().substring(4))) {
						wordBean.replaceAllImageNew("{" + entry.getKey() + "}", entry.getValue().toString().substring(4));
					}
					wordBean.replaceAllText("{" + entry.getKey() + "}", "");
				} else {
					wordBean.replaceAllText("{" + entry.getKey() + "}", entry.getValue().toString());
				}
			}
			wordBean.save(file);
			wordBean.closeDocument();
	...
	public static void main(String[] args) {
		...
		// replaceStr("hhh",new HashMap(),"fff");
