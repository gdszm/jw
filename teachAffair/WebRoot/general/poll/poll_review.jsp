...
<script type="text/javascript">
	...
	//查询
	function _search() {
		gridReview.datagrid('load', dj.serializeObject($('#poll_queryForm')));
		 $('#win_poll_query').window('close');
	}
	//重置
	function cleanSearch() {
		gridReview.datagrid('load', {});
		$('#poll_queryForm input').val('');
	}
	
	//查看社情民意
	function lookPoll(){
		var selectRow = gridReview.datagrid('getSelected');
		if (selectRow) {
	...
	$(document).ready(function() {
		gridReview=$('#gridReview');
		gridReview.datagrid({
			url:'${pageContext.request.contextPath}/poll!datagridReview.do',
			width: 'auto',
			height:'auto',
			striped:true,
			nowrap:true,
			...
			fit:true,
			frozenColumns:[[
				{field:'pollId',title:'操作',align:'center',width:60,sortable:false,formatter:rowformater},
				...
				{field:'title',title:'标题',halign:'center',align:'left',width:480,sortable:true,formatter:function(val,rec){
					var str=val;	
					if(val==undefined){}
					if (rec.status=='<%=Constants.CODE_TYPE_POLL_STATUS_WBL%>'&& rec.reviewFlg=='<%=Constants.CODE_TYPE_YESNO_YES%>'){
	...
	function rowformater(value,row,index){
    	if(row.status=='<%=Constants.CODE_TYPE_POLL_STATUS_STZ%>'){
    		return '<a href="javascript:void(0)" onclick="info('+row.pollId+')">申退理由</a>';
    	}else{
    		return "";
    	}
    }
    ...
    function info(pollId){
   		 p = dj.dialog({
				title : '申退理由',
				href : '${pageContext.request.contextPath}/pollHandle!pollBackInfo.do?poll.pollId='+pollId,
				width : 620,
				height : 430,
				iconCls:'icon-save',
				buttons : [ {
	...
	//调整单位
	function change() {
		var rows = gridReview.datagrid('getSelections');
		if (rows.length == 1) {
			
			if(rows[0].status!='<%=Constants.CODE_TYPE_POLL_STATUS_STZ%>'){
				parent.dj.messagerAlert('提示', '选中记录未进行申退办理不能进行调整单位操作!', 'error');
				return ;
			}
			p = dj.dialog({
				title : '社情民意转办',
	...
	function handleAddSave(){
		...
		$('#confirmForm').form('submit', {
			url : '${pageContext.request.contextPath}/pollHandle!hangleChange.do',
			success : function(d) {
    ...
    //办复审查
	function check() {
		var row = gridReview.datagrid('getSelected');
		if (row) {
			if(row.status=='<%=Constants.CODE_TYPE_POLL_STATUS_YBL%>'){
				dj.messagerAlert('提示', '选中记录办办审查已通过！', 'error');
				return;
			}
			window.open("${pageContext.request.contextPath}/pollHandle!reviewCheck.do?poll.pollId="+row.pollId);
		}else{
	...
	function refuse() {
		...
		if (row) {
			if(row.status!='<%=Constants.CODE_TYPE_POLL_STATUS_STZ%>'){
				dj.messagerAlert('提示', '当前记录没有进行申退！', 'error');
				return;
			}
			parent.dj.messagerConfirm('请确认', '您确定要拒绝申退所选社情民意?', function(r) {
				if (r) {
					$.ajax({
						url : '${pageContext.request.contextPath}/pollHandle!pollRefuse.do?poll.pollId='+row.pollId,
						success : function(d) {
							var json = $.parseJSON(d);
							if (json.success) {
								gridReview.datagrid('reload');
...
<body>
	...
	<div id="tb" style="padding:2px;height:auto">
		<div style="margin-bottom:1px">
			...
			<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-reply" plain="true" onclick="check()">办复审查</a>
			<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="change()">调整单位</a>
			<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-refuse" plain="true" onclick="refuse()">拒绝申退</a>
