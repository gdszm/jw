...
<script type="text/javascript">
	...
	$(document).ready(function(){  
		...
		gridoptions={
			...
			striped:true,
			frozenColumns : [[
				{title:'项目',field:'item',width:170,rowspan:3}				
			]],
			columns:[[
			...
			[
				...
				{title:'办复数',field:'handleNum',align:'right',width:80},
				{title:'办复比例',field:'percent2',align:'right',width:80,formatter:function(val){
					if(val==null||val==''){
						return '';
					}else{
						return val+'%';
		...
		if(selectRows.length > 0){
			if (selectRows.length<6){
				for(var i=0; i<selectRows.length; i++){
					...
					columns1.push("{title:'"+selectRows[i].secondayName+"',align:'center',colspan:3}");
					columns2.push("{title:'提案数',field:'value"+(i)+"',align:'right',width:80}");
					columns2.push("{title:'办复数',field:'handleNum"+(i)+"',align:'right',width:80}");
					columns2.push("{title:'办复比例',field:'percent"+(i)+"',align:'right',width:80,formatter:function(val){if(val==null||val==''){return '0%';}else{return val+'%';}}}");
				}
			}else{
					...
					<table id="formTable">
						<tr>
							...
							<form id="statItemForm" class="form-horizontal" action="/zxta/tjfx/blstat" method="post"></span><br/>
								<span><input id="ybtj_itemid1" name="id" style="height:19px;vertical-align:middle" type="radio" checked="checked" value="COMPANYTYPE"/><label for="ybtj_itemid1"> 单位类别</label></span><br/>
								<span><input id="ybtj_itemid2" name="id" style="height:19px;vertical-align:middle" type="radio" value="BLLX"/><label for="ybtj_itemid2"> 办理方式</label></span><br/>
								<!--<span><input id="ybtj_itemid3" name="id" style="height:19px;vertical-align:middle" type="radio" value="JJCD"/><label for="ybtj_itemid3"> 解决程度</label></span><br/>
								<span><input id="ybtj_itemid4" name="id" style="height:19px;vertical-align:middle" type="radio" value="WYYJ"/><label for="ybtj_itemid4"> 提案人意见</label></span><br/>
								<!--<span><input id="ybtj_itemid5" name="id" style="height:19px;vertical-align:middle" type="radio" value="YESNO"/><label for="ybtj_itemid5"> 是否落实</label></span><br/>  -->
							</form>
							</td>
						</tr>
					</table>
