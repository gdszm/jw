...
var activity;
	$(document).ready(function() {
		...
		getcombobox('status','${pageContext.request.contextPath}/dic!combox.do?ctype=HSTATUS');
		activity=$('#Activitygrid');
		activity.datagrid({
			url:'${pageContext.request.contextPath}/activity!queryDatagrid.do',
			width: 'auto',
			height:'auto',
			striped:true,
			nowrap:true,
			...
			columns:[[
				{field:'timebeg',title:'活动时间',align:'center',width:240,sortable:true,formatter : function(value,row){
	...
	function _search() {
		activity.datagrid('load', dj.serializeObject($('#act_queryForm')));
		//$('#act_queryForm').form('clear');
		 $('#win_act_query').window('close');
	}
	//重置
	function cleanSearch() {
		activity.datagrid('load', {});
		$('#jcgrid').datagrid('load', {});
		$('#act_queryForm input').val('');
...
<body>
	...
	<div id="tb">
		<div style="margin-bottom:1px">
				<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-search" plain="true" onclick="javascript:$('#win_act_query').window('open');">查询</a>
				<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-view" plain="true" onclick="view()">查看详细</a>
		</div>
	</div>
	<div id="win_act_query" class="easyui-window" data-options="title:'查询设置',iconCls:'icon-search',modal:true,collapsible:false,maximizable:false,minimizable:false,closed:true" style="width:630px;height:300px;padding:5px;">
		<div class="easyui-layout" data-options="fit:true">
			<div data-options="region:'center',border:false" style="padding:10px;background:#fff;border:1px solid #ccc;">
				<form id="act_queryForm" enctype=multipart/form-data method="post">
					<table id="dolog_queryTable" >
						<tr height="25">
							<td nowrap>活动种类：</td>
							<td><input type="text" name="aspecies" id="aspecies" style="width:280px;padding:2px;border:1px solid #000;"/></td>
						...
						<tr height="25">
							<td nowrap>活动主题：</td>
							<td colspan="3"> <input type="text" name="atheme" class="easyui-validatebox" style="width:275px;padding:2px;"/></td>
							
						</tr>				
						
						<tr height="25">
							<td nowrap>活动地点：</td>
							<td colspan="3">  <input type="text" name="place" class="easyui-validatebox" style="width:275px;padding:2px;"/></td>
						</tr>
						<tr height="25">
							<td nowrap>承办机构：</td>
							<td colspan="3">  <input type="text" name="agency" id="agency" class="easyui-validatebox" style="width:275px;padding:2px;"/></td>
						</tr>
						<tr height="25">
							<td nowrap>活动日期：</td>
							<td align="left" colspan="3"> <input class="easyui-datebox" name="timebeg"   style="width:134px"/>--<input class="easyui-datebox" name="timeend"  style="width:134px"/></td>
						</tr>
						<tr height="25">
							<td nowrap>状态：</td>
							<td> <input id="status" panelHeight="100px" name="status" style="width:280px;padding:2px;border:1px solid #000;"></td>
						</tr>
					</table>
				</form>
			</div>
			<div data-options="region:'south',border:false" style="text-align:right;padding:5px 0;height:40px">
				...
				<a class="easyui-linkbutton" data-options="iconCls:'icon-cancel'" href="javascript:void(0)" onclick="javascript:$('#win_act_query').window('close');">取消</a>
			</div>
		</div>
	</div>
</body>
