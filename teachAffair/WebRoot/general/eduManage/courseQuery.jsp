...
<script type="text/javascript" charset="utf-8">
	function initCombox(form){
		...
		initCombobox(form,'courseType','${pageContext.request.contextPath}/dic!combox.do?ctype=NOTICETYPE');
	...
	$(function(){
		...
		initCombox("courseDatagrid");
		initCombox("course_queryForm");
		datagrid = $('#courseDatagrid').datagrid({
			url:"${pageContext.request.contextPath}/course!datagridQuery.do",
        	width: 'auto',
			height:'auto',
			striped:true,
			nowrap:true,
			...
			frozenColumns:[[
				...
				{field:'title',title:'课程标题',width:400,align:'center'}
				]],
			columns:[[
				//{field:'content',title:'课程内容',width:100,align:'center'},
		...
		function view() {
			...
			if (rows.length == 1) {
				var p = dj.dialog({
					title : '课程详情',
					href : '${pageContext.request.contextPath}/course!get.do?id='+rows[0].id,
					maximized:true,
					maximizable:true,
					width : 800,
					height : 600,
					onLoad : function() {
						initCombox('courseViewForm');
						var f = p.find('form');
						f.form('load', {
	...
	function cleanSearch() {
		...
		$('#courseTypeId').combobox('setValue', '');
...
<body class="easyui-layout" data-options="fit:true">
	...
	<div id="win_course_query" class="easyui-window" data-options="title:'查询设置',iconCls:'icon-search',modal:true,collapsible:false,maximizable:false,minimizable:false,closed:true" style="width:700px;height:300px;padding:5px;">
...
<!-- 					</tr> -->
					...
					<tr height="30">
						<td nowrap>课程类型 ：</td>
						<td colspan="3"> <input id="courseTypeId"  name="courseType" style="width:300px;"></td>
						<td/>
					</tr>
					<tr height="30">
						<td nowrap>创建日期 ：</td>
			...
			<div data-options="region:'south',border:false" style="text-align:right;padding:5px 0;height:40px">
				...
				<a class="easyui-linkbutton" data-options="iconCls:'icon-cancel'" href="javascript:void(0)" onclick="javascript:$('#win_course_query').window('close');">取消</a>
			</div>
		</div>
	</div>
</body>
