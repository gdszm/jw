...
<script type="text/javascript" charset="utf-8">
	...
	$(function(){
		...
		datagrid = $('#archDatagrid').datagrid({
			url:'${pageContext.request.contextPath}/archives!datagrid.do',
        	width: 'auto',
			height:'auto',
			striped:true,
			nowrap:true,
			...
			frozenColumns:[[
				...
				{field:'archNoV',title:'档案编号',width:100,align:'center',
			...
			columns:[[
				{field:'urgentTel',title:'紧急情况下联系号码',width:100,align:'center'},
				{field:'gradSchool',title:'毕业院校',width:150,align:'center'},
				{field:'crtTime',title:'创建时间',width:150,align:'center'},
				{field:'uptTime',title:'更新时间',width:100,align:'center'},
				{field:'baseInfoId',title:'人员基本信息编号',width:100,align:'center'},
				{field:'id',title:'家庭信息编号',width:100,align:'center'},
				{field:'statusName',title:'发布状态',width:100,align:'center'}
			]], 
			toolbar : [
			{
				text : '查询',
		...
		function view() {
			...
			if (rows.length == 1) {
				var p = dj.dialog({
					title : '档案详情',
					href : '${pageContext.request.contextPath}/archives!archivesView.do?archNo='+rows[0].archNo,
					width : 700,
					height : 420,
					maximized : true,
					onLoad : function() {
					...
					//	initCombox('weForm');
						//家庭成员列表
						gettjzgrid('${pageContext.request.contextPath}/archives!fmDatagrid.do?archNo=' + rows[0].archNo);
						//奖惩记录信息列表
						getApGrid('${pageContext.request.contextPath}/archives!apDatagrid.do?archNo=' + rows[0].archNo);
						//教育经历列表
						getEeGrid('${pageContext.request.contextPath}/archives!eeDatagrid.do?archNo=' + rows[0].archNo);
						//家庭成员信息列表
						getWeGrid('${pageContext.request.contextPath}/archives!weDatagrid.do?archNo=' + rows[0].archNo);
...
// 	}
	...
	function edit() {
		...
		if (rows.length == 1) {
			var p = dj.dialog({
				title : '编辑档案信息',
				href : '${pageContext.request.contextPath}/archives!archivesEdit.do?archNo='+rows[0].archNo,
				width : 700,
				height : 420,
				maximized : true,
				buttons : [ {
					...
					handler : function() {
							...
							var f = p.find('form');
							f.form('submit', {
								url : '${pageContext.request.contextPath}/archives!edit.do',
								success : function(d) {
...
function gettjzgrid(url){
		$('#submittergrid').datagrid({
			...
			columns:[[
				...
				{field:'unit',title:'工作单位',align:'center',width:120,sortable:false},
...
function getApGrid(url){
		$('#apGrid').datagrid({
			...
			columns:[[
				...
				{field:'apMethodName',title:'奖惩方式',align:'center',width:120,sortable:false},
...
function getEeGrid(url){
		$('#eeGrid').datagrid({
			...
			columns:[[
				...
				{field:'cert',title:'所获证书',align:'center',width:120,sortable:false},
				{field:'certifier',title:'证明人',align:'center',width:120,sortable:false},
				{field:'crtTime',title:'创建时间',align:'center',width:200,sortable:false},
				{field:'uptTime',title:'更新时间',align:'center',width:200,sortable:false}
			]],
			toolbar: '#ee_menu'
			...
			parent.dj.messagerConfirm('请确认', '您要删除当前所选项目？', function(r) {
				if (r) {
					for ( var i = 0; i < rows.length; i++) {
						ids.push(rows[i].archNo);
					}
					$.ajax({
						url : '${pageContext.request.contextPath}/archives!delete.do',
						data : {
							ids : ids.join(',')
						},
						dataType : 'json',
		...
		if (rows.length > 0) {
			parent.dj.messagerConfirm('请确认', '您要发布当前所选档案？', function(r) {
				if (r) {
					...
					$.ajax({
						url : '${pageContext.request.contextPath}/archives!pub.do',
						data : {
							ids : ids.join(',')
						},
						dataType : 'json',
		...
		if (rows.length > 0) {
			parent.dj.messagerConfirm('请确认', '您要取消发布当前所选档案？', function(r) {
				if (r) {
					...
					$.ajax({
						url : '${pageContext.request.contextPath}/archives!cancelPub.do',
						data : {
							ids : ids.join(',')
						},
						dataType : 'json',
...
<body class="easyui-layout" data-options="fit:true">
	...
	<div data-options="region:'center',border:false" style="overflow: hidden;">
		<table id="archDatagrid"></table>
	</div>
	<div id="win_notice_query" class="easyui-window" data-options="title:'查询设置',iconCls:'icon-search',modal:true,collapsible:false,maximizable:false,minimizable:false,closed:true" style="width:470px;height:375px;padding:5px;">
		<div class="easyui-layout" data-options="fit:true">
			<div data-options="region:'center',border:false" style="padding:10px;background:#fff;border:1px solid #ccc;">
				...
				<table id="poll_queryTable">
					<tr height="30">
						<td nowrap>档案编号：</td>
						<td colspan="3"><input type="text" class="easyui-validatebox"  name="archNo" style="width:300px;"></td>
					</tr>
					<tr height="30">
						<td nowrap>档案人姓名：</td>
					...
					<tr height="30">
						<td nowrap>性别：</td>
						<td colspan="3">
							<input type="text" class="easyui-validatebox"  name="sex" style="width:302px;">
						</td>
					</tr>
					<tr height="30">
						<td nowrap>紧急联系电话：</td>
						<td colspan="3"><input type="text" class="easyui-validatebox"  name="urgentTel" style="width:300px;"></td>
					</tr>
					<tr height="30">
						<td nowrap>毕业院校：</td>
						<td colspan="3"><input type="text" class="easyui-validatebox"  name="gradSchool" style="width:300px;"></td>
					</tr>
					<tr height="30">
						<td nowrap>创建时间范围 ：</td>
						<td colspan="3">
							<input id="crtSTimeId" type="text" class="easyui-datebox" name="crtSTime" data-options="required:false,showSeconds:false" style="width:142px;"></input> 
		 	              	~ 
			              	<input id="crtETimeId" type="text" class="easyui-datebox" name="crtETime" data-options="required:false,showSeconds:false" style="width:142px;"></input>  
						</td>
					</tr>
					<tr height="30">
						<td nowrap>更新时间范围 ：</td>
						<td colspan="3">
							<input id="uptSTimeId" type="text" class="easyui-datebox" name="uptSTime" data-options="required:false,showSeconds:false" style="width:142px;"></input> 
		 	              	~
			              	<input id="uptETimeId" type="text" class="easyui-datebox" name="uptETime" data-options="required:false,showSeconds:false" style="width:142px;"></input>  
						</td>
					</tr>
					<tr height="30">
						<td nowrap>备注：</td>
