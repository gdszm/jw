...
<script type="text/javascript" charset="utf-8">
	...
	$(function(){
		...
		initCombox("metfileDatagrid");
		initCombox("meetFile_queryForm");
		var statusPubYes='<%=Constants.CODE_TYPE_PUBSTATUS_YES%>';
		var url='${pageContext.request.contextPath}/meetFile!datagrid.do?fromtype=1&status='+statusPubYes;
		datagrid = $('#metfileDatagrid').datagrid({
			url:url,
        	width: 'auto',
			height:'auto',
			striped:true,
			...
			toolbar : [
			{
				...
				iconCls : 'icon-search',
				handler : function() {
					$('#win_meetfile_query').window('open');
				}
			}, '-',
			{
				text : '查看详细',
	...
	function _search() {
		datagrid.datagrid('load', dj.serializeObject($('#meetFile_queryForm')));
		 $('#win_meetfile_query').window('close');
	}
...
<body class="easyui-layout" data-options="fit:true">
	...
	<div data-options="region:'center',border:false" style="overflow: hidden;">
		<table id="metfileDatagrid"></table>
	</div>
	<div id="win_meetfile_query" class="easyui-window" data-options="title:'查询设置',iconCls:'icon-search',modal:true,collapsible:false,maximizable:false,minimizable:false,closed:true" style="width:500px;height:274px;padding:5px;">
		<div class="easyui-layout" data-options="fit:true">
			<div data-options="region:'center',border:false" style="padding:10px;background:#fff;border:1px solid #ccc;">
				...
				<table id="poll_queryTable">
					<tr height="25">
						<td nowrap>文件类型：</td>
						<td colspan="3"><input id="noticeTypeId" name="datatype" style="width:302px;"></td>
					</tr>
					<tr height="25">
						<td nowrap>文件标题：</td>
			...
			<div data-options="region:'south',border:false" style="text-align:right;padding:5px 0;height:40px">
				...
				<a class="easyui-linkbutton" data-options="iconCls:'icon-cancel'" href="javascript:void(0)" onclick="javascript:$('#win_meetfile_query').window('close');">取消</a>
			</div>
		</div>
	</div>
</body>
