...
<script type="text/javascript" charset="utf-8">
	function initCombox(form){
		...
		initCombobox(form,'classroomType','${pageContext.request.contextPath}/dic!combox.do?ctype=NOTICETYPE');
	...
	$(function(){
		...
		initCombox("classroomDatagrid");
		initCombox("classroom_queryForm");
		datagrid = $('#classroomDatagrid').datagrid({
			url:"${pageContext.request.contextPath}/classroom!datagridQuery.do",
        	width: 'auto',
			height:'auto',
			striped:true,
			nowrap:true,
			...
			columns:[[
				...
				{field:'classroomTypeName',title:'班级类型',width:100,align:'center'},
			...
			if (rows.length == 1) {
				var p = dj.dialog({
					...
					onLoad : function() {
						...
						var f = p.find('form');
						f.form('load', {
							classroomNo : rows[0].classroomNo,
							classroomName : rows[0].classroomName,
							classroomType: rows[0].classroomType,
	...
	function cleanSearch() {
		...
		$('#classroomTypeId').combobox('setValue', '');
...
<body class="easyui-layout" data-options="fit:true">
	...
	<div id="win_classroom_query" class="easyui-window" data-options="title:'查询设置',iconCls:'icon-search',modal:true,collapsible:false,maximizable:false,minimizable:false,closed:true" style="width:700px;height:300px;padding:5px;">
...
<!-- 					</tr> -->
					...
					<tr height="30">
						<td nowrap>班级类型 ：</td>
						<td colspan="3"> <input id="classroomTypeId"  name="classroomType" style="width:300px;"></td>
						<td/>
					</tr>
					<tr height="30">
						<td nowrap>创建日期 ：</td>
