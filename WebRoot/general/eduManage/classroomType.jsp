...
<script type="text/javascript" charset="utf-8">
	function initCombox(form){
		//initCombobox(form,'type','${pageContext.request.contextPath}/dic!combox.do?ctype=classroomType');
		...
		if (rows.length > 0) {
			parent.dj.messagerConfirm('请确认', '您要发布当前所选班级？', function(r) {
				if (r) {
					...
					$.ajax({
						url : '${pageContext.request.contextPath}/classroom!pub.do',
						data : {
							ids : ids.join(',')
						},
						dataType : 'json',
		...
		if (rows.length > 0) {
			parent.dj.messagerConfirm('请确认', '您要取消发布当前所选班级？', function(r) {
				if (r) {
					...
					$.ajax({
						url : '${pageContext.request.contextPath}/classroom!cancelPub.do',
						data : {
							ids : ids.join(',')
						},
						dataType : 'json',
